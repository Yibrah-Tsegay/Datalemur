/*
  ...
  */ 
        SELECT 
            b.yr, b.mth, b.mtm AS growth_pct
      FROM 
            (SELECT 
                  a.yr AS yr, a.mth AS mth, a.current_duration AS cd,
                  LAG(a.clc) OVER (ORDER BY 
                  a.mth) AS llc,
                  a.clc AS clc,
                  ROUND((100.0*(a.clc - LAG(a.clc) OVER (ORDER BY 
                  a.mth))/(LAG(a.clc) OVER (ORDER BY 
                  a.mth))::DECIMAL),1) AS mtm
            FROM 
                  (SELECT 
                        EXTRACT(year FROM call_received) AS yr,
                        EXTRACT(month FROM call_received) AS mth,
                        COUNT(case_id) As clc,
                        SUM(call_duration_secs) AS current_duration
                  FROM 
                        callers
                  WHERE 
                  ...
                  ORDER BY 
                         EXTRACT(month FROM call_received)) a) b
/* 
    Resulting Table: 
